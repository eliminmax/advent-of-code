...
// Solution to AoC 2018 Day 13 Part 1
...
fn main() {
    ...
    let mut new_locs: HashSet<Location> = HashSet::new();
    'processing_loop: loop {
        for elf in elves.iter_mut() {
            let _ = new_locs.remove(&elf.location);
            elf.advance(&tracks);
            if !new_locs.insert(elf.location) {
                println!("{},{}", elf.location.x, elf.location.y);
