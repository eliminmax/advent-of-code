...
* instead of setting a temporary value in each branch of a conditional, then after the conditional, setting a final variable to the temporary one, the final value is set directly.
...
Viewing the generated C code in an editor with LSP support, with `clangd` running, it immediately flagged one of the register variables as unused - namely, `r4`, which was bound to the instruction pointer. I removed that register variable.
...
The first step was to replace the starting `L05` and ending `if (!r3) goto L05;` with a `do ... while` loop.
