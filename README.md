...
At the start of my efforts, I primarily used Python for my solutions, or occasionally AWK or Rust, but as I worked my way through, I found myself using Rust more and more, and occasionally using C as well. The one time that the problem involved filtering JSON data, I used jq, and when AWK seemed to me to be the best tool for the job, I use it, but that's rare.

Almost every solution is in a single language, in a standalone source file, with dependencies only on its language's standard library and occasionally other source files in the same directory. The most common exception is reverse-engineering/decompilation problems that have sometimes come up, for which I'll use a mix of tools to poke at the problem from different angles, typically keeping notes in a Markdown file.
