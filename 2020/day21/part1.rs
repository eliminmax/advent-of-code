...
// Solution to AoC 2020 Day 21 Part 1
...
fn main() {
    ...
    let mut allergens = StaticStrSet::with_capacity(unresolved_allergens.len());
    ...
    let mut safe_ingredient_counts: HashMap<&'static str, u32> = ingredients
        .difference(&allergens)
        .map(|&s| (s, 0))
        .collect();
    for line in INPUT.lines() {
        let (ingredients, _) = line.split_once(" (contains").unwrap();
        for ingredient in ingredients.split_ascii_whitespace() {
            if let Some(count) = safe_ingredient_counts.get_mut(ingredient) {
                *count += 1;
            }
        }
    }

    let sum: u32 = safe_ingredient_counts.into_values().sum();
    println!("{sum}");
}
