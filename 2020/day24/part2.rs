...
// Solution to AoC 2020 Day 24 Part 2
...
fn next_day(current_black: HashSet<HexPoint>) -> HashSet<HexPoint> {
    // start with any currently black tile, as well as all neighbors of currently-black tiles.
    let mut next_gen: HashSet<HexPoint> = current_black
        .iter()
        .cloned()
        .flat_map(|tile| tile.neighborhood())
        .chain(current_black.iter().cloned())
        .collect();
    ...
    next_gen.retain(|pos| {
        let n = pos
            .neighborhood()
            .into_iter()
            .filter(|n| current_black.contains(n))
            .count();
        if current_black.contains(pos) {
            n == 1 || n == 2
        } else {
            n == 2
        }
...
impl std::ops::Add<HexDir> for HexPoint {
    type Output = Self;
    fn add(self, rhs: HexDir) -> Self {
        match rhs {
            HexDir::E => Self {
                q: self.q - 1,
                ..self
