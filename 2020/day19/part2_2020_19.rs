...
// Solution to AoC 2020 Day 19 Part 2
use std::collections::{HashMap, HashSet, VecDeque};
#[derive(Debug)]
struct ResolvedRuleZero {
    rule42: HashSet<String>,
    rule31: HashSet<String>,
}

impl ResolvedRuleZero {
    fn resolve_rule_0(rule_text: &str) -> Self {
        ...
        for line in rule_text.lines() {
            ...
            if num == 0 {
                assert_eq!(rule, "8 11", "rule 0 must be \"8 11\" for solve to work");
                continue;
            }
            if num == 8 {
                assert_eq!(rule, "42");
        ...
        while let Some(id) = unresolved.pop_front() {
            ...
            macro_rules! get_entry {
                ($k: ident) => {{
                    assert!(
                        !matches!($k, 0 | 8 | 11),
                        "Can't solve with dependencies on special rules 0, 8, or 11"
                    );
    ...
    fn matches(&self, mut line: &str) -> bool {
        // rule 0 is `8 11`
        //
        // rule 8 is `42 | 42 8`, which means one or more matches of rule 42
        //
        // rule 11 is `42 31 | 42 11 31`, which means one or more matches of rule 42, followed by
        //     the same number of matches of rule 31.
        //
        // so that means that rule 0 is two or more matches of rule 42, followed by 1 or more
        // matches of rule 31, with fewer matches of rule 31 than rule 42.
        ...
        loop {
            ...
            if let Some(prefix) = self
                .rule42
                .iter()
                .find(|pat| line.starts_with(pat.as_str()))
            {
                // SAFETY: leading match was already found and is known to exist
                line = unsafe { line.strip_prefix(prefix).unwrap_unchecked() };
                match42 += 1;
                any_matched = true;
            }

            if let Some(suffix) = self.rule31.iter().find(|pat| line.ends_with(pat.as_str())) {
                // SAFETY: trailing match was already found and is known to exist
                line = unsafe { line.strip_suffix(suffix).unwrap_unchecked() };
                match31 += 1;
                any_matched = true;
            }

            if line.is_empty() {
                return match31 >= 1 && match42 > match31;
            }
...
fn main() {
    ...
    let rule_0 = ResolvedRuleZero::resolve_rule_0(rule_text);

    let valid_rule_count = patterns.lines().filter(|line| rule_0.matches(line)).count();
