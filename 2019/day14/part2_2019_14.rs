...
// Solution to AoC 2019 Day 14 Part 2
...
fn needed_ore<'a>(
    ...
    while let Some((item_to_make, mut amount_to_make)) = needed.pop_last() {
        ...
        inventory.entry(item_to_make).and_modify(|held_amnt| {
...
fn main() {
    let mut available_ore: u64 = 1_000_000_000_000;
    let mut inventory: HashMap<&'static str, u64> = HashMap::new();
    let recipes: HashMap<&'static str, Recipe<'static>> = parse_recipes(include_str!("input"));
    let mut total_fuel: u64 = 0;
    loop {
        let needed = needed_ore(&recipes, &mut inventory);
        if needed > available_ore {
            break;
        }
        available_ore -= needed;
        total_fuel += 1;
    }
    println!("{total_fuel}");
}
