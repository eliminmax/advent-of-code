...
// Solution to AoC 2015 Day 21 Part 2
...
/// Return Some(cost) if inventory would result in loss, otherwise None
...
fn main() {
        ...
        for armor in ARMORS.iter() {
            for rings in RingChoices::default() {
                ...
                if let Some(cost) = test_inventory(&boss_stats, inventory) {
                    if max_cost.is_none_or(|old_max| old_max < cost) {
                        max_cost = Some(cost);
                    }
                }
            }
    ...
    println!("{}", max_cost.expect("All inventories could beat the boss"));
}
