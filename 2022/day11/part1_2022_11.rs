...
// Solution to AoC 2022 Day 11 Part 1
...
impl MonkeyOp {
    const fn apply(self, worry: u32) -> u32 {
        match self {
...
#[derive(Debug)]
struct Monkey {
    inventory: RefCell<VecDeque<u32>>,
...
impl Monkey {
    fn toss(&self) -> impl IntoIterator<Item = MonkeyToss> {
        let mut v = Vec::new();
...
fn run_round(monkeys: &[Monkey]) {
    for monkey in monkeys {
        for MonkeyToss { worry, target } in monkey.toss() {
