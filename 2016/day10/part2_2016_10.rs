...
// Solution to AoC 2016 Day 10 Part 2
...
impl BotSystem {
    ...
    fn simulate(&mut self) -> Result<(), ProcessingError> {
        ...
        while let Some(id) = queue.pop_front() {
            let rule = self.bots[id].rule.ok_or(ProcessingError)?;
...
fn main() {
    ...
    system.simulate().expect("Failed to fully simulate system");
    let leading_outputs: Vec<Vec<u8>> = system.outputs.into_iter().take(3).collect();
    assert_eq!(
        leading_outputs.iter().map(|o| o.len()).collect::<Vec<_>>(),
        vec![1; 3]
    );
    println!(
        "{}",
