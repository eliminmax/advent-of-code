...
// Solution to AoC 2024 Day 18 Part 1
...
fn main() {
    ...
    grid[0][0] = MemSpace::Safe(Some(0));
    dijkstra_score(&mut grid);
    if let MemSpace::Safe(Some(dist)) = grid[70][70] {
        println!("{dist}");
    } else {
        panic!("grid[70][70] = {:?}", grid[70][70]);
    }
}
