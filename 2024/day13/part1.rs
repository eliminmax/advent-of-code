...
// Solution to AoC 2024 Day 13 Part 1
    ...
    fn from_str(s: &str) -> Result<ArcadeMachine, ParseArcadeError> {
        ...
        match words.as_slice() {
            [
                ...
                let px = extract_u64!(px, "X=", ",")?;
                let py = extract_u64!(py, "Y=")?;
                Ok(ArcadeMachine {
                    a: Button { x: ax, y: ay },
                    b: Button { x: bx, y: by },
...
fn main() {
    ...
    // let machines: Vec<ArcadeMachine> = input
    //     .split("\n\n")
    //     .map(|m| ArcadeMachine::from_str(m).expect("Passed an invalid arcade machine string"))
    //     .collect();
    // let tokens: u64 = machines.iter().map(|machine| machine.token_count()).sum();
    println!(
        "{}",
        input
            .split("\n\n")
            .map(|m| ArcadeMachine::from_str(m)
                .expect("Passed an invalid arcade machine string")
                .token_count())
            .sum::<u64>()
    );
}
