...
check_offsets: tuple[OffsetGroup] = (
    # M on top
...
def is_cross_at(row: int, col: int, grid: Iterable[Iterable[str]]) -> bool:
    if grid[row][col] != "A":
        return False
    for offs in check_offsets:
        a, b = offs.m_offs
        if grid[row + a[0]][col + a[1]] != "M" or grid[row + b[0]][col + b[1]] != "M":
            continue
        # first 2 corners checked are "M", so if the other 2 aren't "S",
        # there's no need to keep checking other rotations
        c, d = offs.s_offs
        return bool(grid[row + c[0]][col + c[1]] == "S" and grid[row + d[0]][col + d[1]] == "S")
    return False
