...
impl fmt::Display for WarehouseFloor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut grid_string: String = String::new();
        self.grid.iter().for_each(|l| {
            l.iter().for_each(|gs| match gs {
                GridSpace::Empty => grid_string += "\x1b[30;2m.",
                GridSpace::Bot => grid_string += "\x1b[1;32m@",
                GridSpace::Wall => grid_string += "\x1b[37;1;2m#",
                GridSpace::BoxLeft => grid_string += "\x1b[1;33m[",
                GridSpace::BoxRight => {
                    if grid_string.ends_with("[") {
                        grid_string += "]";
                    } else {
                        grid_string += "\x1b[1;93;41m]\x1b[49m"
                    }
                }
            });
            grid_string += "\x1b[m\n";
        ...
        self.bot_moves.iter().take(100).for_each(|&m| match m {
            BotMove::Up => moves_string += "\x1b[31m^",
            BotMove::Right => moves_string += "\x1b[34m>",
            BotMove::Down => moves_string += "\x1b[35mv",
            BotMove::Left => moves_string += "\x1b[36m<",
        });
        moves_string += "\x1b[m";
