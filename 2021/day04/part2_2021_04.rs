...
// Solution to AoC 2021 Day 4 Part 2
...
fn main() {
    ...
    let mut boards: Vec<BingoBoard> = line_groups
        ...
        .collect();
    'bingo: for num in nums.by_ref() {
        boards.retain_mut(|board| board.mark(num).is_none());
        if boards.len() <= 1 {
            break 'bingo;
        }
    }
    if let Some(mut board) = boards.pop() {
        'remaining: for num in nums {
        ...
        panic!("No single board left over!");
    }
}
